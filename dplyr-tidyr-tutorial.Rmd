...
asfis <- read_csv(file = 'data/ASFIS_Feb2014.csv')
...
```{r,eval=F,echo=T}
d<- d %>%
  group_by(country,sciname,commname,regionfao) %>%
  complete(year = 1950:2012, nesting(country,sciname,regionfao,commname,spgroupname,spcode)) 
```
...
# left join to retain all data in our d data frame. We get additional rows because there are multiple lh entries for certain species
d %>% 
...
One of the best aspects of working with tidy data and `dplyr` is how easy it makes it to quickly manipulate and plot your data. Property oroganized, it's a piece of cake to quickly make summaries and plots of your data without making all kinds of "temporary" files or lines of spagheti code for plotting. You can also basically eliminate loops from your coding for all situations except that those that reuiqre dynamic updating (e.g. population models). 
...
You want to find a fishery to join based on two criteria: high average catch, and low average variability. We are first going to. You might now know these data though, so you want to be able to predict what fishery to join based on geographic and life history traits. 
...
reg_vars <- c('regionfao', 'spgroupname', 'vbk','temp','maxl','agem') #specify variables you want

class(regdat$regionfao) #whoops, it things FAO region is an integer, we want a factor

regdat$regionfao <- as.factor(regdat$regionfao)

reg_fmla <- as.formula(paste('fishiness ~',paste(reg_vars, collapse = '+'), sep = '')) #create regression formula

fish_model <- lm(reg_fmla, data = regdat) #run a linear regression
...
tidy_model$variable<- as.factor(tidy_model$term)
  
  tidy_model$variable <- reorder(tidy_model$variable, tidy_model$p.value)
  
  tidy_model$short_pval<- pmin(tidy_model$p.value,0.2)
...
We're now going to use the `augment()` function in the `broom` package to help us run some diagnostics. The `augment` function takes our original data passed to the regression, and adds all kinds of things, like the values predicted by the model and the residuals. 
