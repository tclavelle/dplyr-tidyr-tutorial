...
Lets use a left join to add the life history parameters to our FAO data
```{r,eval=F,echo=T}
...
filtered_dat <- regdat %>%
  ...
  mutate(regionfao = as.factor(regionfao),spgroupname = as.factor(spgroupname),
         has_all = apply(is.na(regdat[,reg_vars]) == F, 1,all) ) %>%
  filter(has_all == T)
...
Looks a little iffy, we've got some heteroskedasticity going on, and the linear model we're using might not be quite right.  Let's try and see where it is. The great thing about `broom` is that it makes it really easy to manipulate data and plot diagnostics based on the original data. 
