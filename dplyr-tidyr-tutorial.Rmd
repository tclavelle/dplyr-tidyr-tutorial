...
+ Can [download RStudio](https://www.rstudio.com/products/rstudio/download/) if you don't have latest version 0.99.892 (menu RStudio -> About RStudio), which has many nice additions for running R chunks and providing table of contents in Rmarkdown documents.
...
# note: read_csv like read.csv(...,stringsAsFactors = F)
...
df <- gather(fao, key='Year', value='Catch', num_range('X',1950:2012)) # ?select for num_range()
# ! df() already a function in R, so d preferred
if (interactive()) View(df) # interactive==T if in Console, not knitting
# to handle: '-','...',' F','X'
...
df <- fao %>%
  ...
  mutate(
    year  = as.numeric(str_replace(year, 'X', '')),  # strip X from years and convert to numeric
    catch = as.numeric(str_replace(catch, c(' F','...','-'), replacement = '')))  %>% # strip ' F' and convert to numeric
  filter(!country %in% c('Totals - Quantity (number)','Totals - Quantity (tonnes)')) %>% # remove 'Totals' rows
  filter(!is.na(catch)) # rows: 1,114,596 -> 310,619
...
df %>% # note: we need to use the new name
  left_join(lh, by='sciname') # we only need to specify the right hand data set to join lh with
```
